...
public class ShaderVariantCollectionMaterialVariantConverter
{
    ...
    public static string[] GetShaderKeywords(Shader shader)
    {
        ...
        /*
        foreach (var item in globalKeywords)
        {
            Debug.LogError("global keyword "+ item+" shader name "+shader.name);
        }
        foreach (var item in localKeywords)
        {
            Debug.LogError("local keyword " + item + " shader name " + shader.name);
        }
        */
    ...
    public  bool IsKeywordBelongToShader(Shader shader, string keyword)
    {
    ...
    public bool CollectVariant()
    {
        ...
        try
        {
            ...
            foreach (var item in mInsertedVariants)
            {
                var shader = item.Key;
                AddVariantFromKeywords(shader, new List<string>());
            }
    ...
    {
        //Debug.LogError("begin WriteToShaderVariantCollectionFile");
        foreach (var variant in mVariants)
        {
          //  Debug.LogError("do WriteToShaderVariantCollectionFile"+ variant.shader.name);
            collection.Add(variant);
        }
        //Debug.LogError("end WriteToShaderVariantCollectionFile");
    }
}
