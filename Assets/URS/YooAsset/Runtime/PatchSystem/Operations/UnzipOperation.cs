...
namespace YooAsset
{
	public class UnzipOperation : AsyncOperationBase
	{
		private enum ESteps
		{
			None,
		...
		private readonly List<UnzipEntry> _downloadList;
		private readonly List<UnzipEntry> _loadFailedList = new List<UnzipEntry>();
		private readonly List<Unziper> _downloaders = new List<Unziper>();
		private readonly List<Unziper> _removeList = new List<Unziper>(MAX_LOADER_COUNT);
		...
		internal UnzipOperation(List<UnzipEntry> downloadList, int fileLoadingMaxNumber, int failedTryAgain)
		{
		...
		internal override void Update()
		{
			...
			if (_steps == ESteps.Loading)
			{
				...
				foreach (var downloader in _downloaders)
				{
                    ...
                    UnzipEntry bundleInfo = downloader.GetBundleInfo();
				...
				if (_downloadList.Count > 0 && _loadFailedList.Count == 0)
				{
					if (_downloaders.Count < _fileLoadingMaxNumber)
					{
						...
						var operation = UnzipSystem.BeginDownload(_downloadList[index], _failedTryAgain);
