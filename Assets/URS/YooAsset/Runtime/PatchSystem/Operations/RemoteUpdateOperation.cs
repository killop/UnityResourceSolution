...
namespace YooAsset
{
	public class RemoteUpdateOperation : AsyncOperationBase
	{
		private enum ESteps
		{
			None,
		...
		private readonly List<UpdateEntry> _downloadList;
		private readonly List<UpdateEntry> _loadFailedList = new List<UpdateEntry>();
		private readonly List<RemoteDownloader> _downloaders = new List<RemoteDownloader>();
		private readonly List<RemoteDownloader> _removeList = new List<RemoteDownloader>(MAX_LOADER_COUNT);
		...
		internal RemoteUpdateOperation(List<UpdateEntry> downloadList, int fileLoadingMaxNumber, int failedTryAgain)
		{
			...
			{
				TotalDownloadBytes += patchBundle.GetRemoteDownloadFileSize();
			}
		}
		...
		internal override void Update()
		{
			...
			if (_steps == ESteps.Loading)
			{
				...
				foreach (var downloader in _downloaders)
				{
					...
					var bundleInfo = downloader.GetUpdateEntry();
					...
					CurrentDownloadBytes += bundleInfo.GetRemoteDownloadFileSize();
				}
				...
				if (_downloadList.Count > 0 && _loadFailedList.Count == 0)
				{
					if (_downloaders.Count < _fileLoadingMaxNumber)
					{
						...
						var operation = RemoteDownloadSystem.BeginDownload(_downloadList[index], _failedTryAgain);
				...
				if (_downloaders.Count == 0)
				{
					...
					{
						string fileName = _loadFailedList[0].GetRelativePath();
