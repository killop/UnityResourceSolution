...
namespace YooAsset
{
	internal class HttpDownloader
	{
		...
		internal HttpDownloader(HardiskFileSearchResult result)
		{
            _hardiskFileSearchResult = result;
		}
		...
		private void ThreadRun()
		{
			...
			string savePath = _hardiskFileSearchResult.HardiskPath;
			long fileTotalSize = _hardiskFileSearchResult.SizeBytes;
			...
			try
			{
				...
				{
					_threadResult = false;
					_threadError = $"Verify file content failed : {_hardiskFileSearchResult.Hash}";
				}
			}
			catch (Exception e)
			{
		...
		private string GetRequestURL()
		{
			// 轮流返回请求地址
			_requestCount++;
            return _hardiskFileSearchResult.UpdateEntry.GetRemoteDownloadURL();
        }
		#endregion
	}
}